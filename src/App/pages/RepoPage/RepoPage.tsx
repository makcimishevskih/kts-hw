...
const RepoPage: FC<TRepoPageProps> = observer(() => {
  ...
  const repo: TOrgRepo | null = (id && OrgStore.findRepoById(id)) || null;

  useEffect(() => {
    if (repo && OrgStore.orgName) {
      OrgStore.getRepoData(FILE_NAME);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [OrgStore.orgName, repo]);
  ...
  return (
    <div className={css.repo}>
      ...
      <div className={css.repo__info}>
        <Contributors
          contributors={OrgStore.contributors}
          error={OrgStore.errorsRepo.contributors}
          loading={OrgStore.loadersRepo.contributors}
        />
        <Languages
          languages={OrgStore.languages}
          error={OrgStore.errorsRepo.languages}
          loading={OrgStore.loadersRepo.languages}
        />
      </div>
      <Readme readme={OrgStore.readme} error={OrgStore.errorsRepo.readme} loading={OrgStore.loadersRepo.readme} />
    </div>
  );
});
export default RepoPage;

// const RepoPage: FC<TRepoPageProps> = observer(({ orgName, repos }) => {
