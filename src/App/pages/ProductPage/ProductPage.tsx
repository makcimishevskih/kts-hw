...
import { getContributors, getTags, TContributor, TOrgs, TTag, TReadme } from 'utils/fetchData';
...
import { base64ToBytes } from 'utils/encode';
...
const ProductPage: FC<IProductPageProps> = ({ list }) => {
  const [contributors] = useState<TContributor[]>(c);
  const [tags] = useState<TTag[]>(t);
  const [readme, setReadme] = useState<TReadme>(r);
  ...
  useEffect(() => {
    // getContributors(product.contributors_url).then(setContributors);
    // getTags(product.tags_url).then(setTags);
    // getReadme(product.contents_url + 'README.md').then(data => {
    //   const decodedContent = new TextDecoder().decode(base64ToBytes(data.content));
  ...
  useEffect(() => {
    ...
    // axios.get('https://api.github.com/repos/ktsstudio/notific/contents/README.md', config).then((res) => {
    //   const encodedContent = res.data.content;
    //   const decodedContent = new TextDecoder().decode(base64ToBytes(encodedContent));
  ...
  const getFormatedText = (text: any) => {
    return text.split('\n').map((str: any, i: number) => <p key={`p_${i}`}>{str}</p>);
  };

  const rr = new TextDecoder().decode(base64ToBytes(r.content));
      ...
      <div className={css.product__info}>
        ...
        <div className={css.product__languages}>
          ...
          <ul className={css.languages__list}>
            ...
            {/* <Text tag="p" className={css.language__item}>
              lang
            </Text> */}
          </ul>
        </div>
